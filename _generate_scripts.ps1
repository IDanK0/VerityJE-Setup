...
if(!`$fk){Write-Host "`nNo API key found in environment." -F Yellow;Write-Host "Enter your API key:" -F Yellow;`$k=Read-Host;if(`$k.Trim()){Set-Item -Path "env:OPENAI_API_KEY" -Value `$k.Trim();Write-Host "Set OPENAI_API_KEY" -F Green}}
...
`$edEnv = if(Test-Path `$ed){"`$env:PHONEMIZER_ESPEAK_LIBRARY='" + $EspeakDll + "';"}else{""}
Start-Process powershell -NoExit -Arg "-NoExit","-Command","`$env:PYTHONUTF8='1';" + `$edEnv + "`$env:MODEL_DIR='" + "`" + "`$repoPath" + "\api\src\models';`$env:VOICES_DIR='" + "`" + "`$repoPath" + "\api\src\voices\v1_0';`$env:PYTHONPATH='" + "`" + "`$repoPath" + ";" + "`$repoPath" + "\api'; & '" + "`$vu" + "' api.src.main:app --host 127.0.0.1 --port 8880" -WindowStyle Minimized
...
function Menu {
  ...
  Write-Host "";Write-Host "-- Services --" -F Yellow
