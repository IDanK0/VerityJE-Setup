...
Start-Process powershell -NoExit -Arg "-NoExit","-Command","$env:PYTHONUTF8='1';if(Test-Path 'C:\Program Files\eSpeak NG\libespeak-ng.dll'){$env:PHONEMIZER_ESPEAK_LIBRARY='C:\Program Files\eSpeak NG\libespeak-ng.dll'};$env:MODEL_DIR='$($env:MODEL_DIR)';$env:VOICES_DIR='$($env:VOICES_DIR)';$env:PYTHONPATH='$($env:PYTHONPATH)'; & '$vu' api.src.main:app --host 127.0.0.1 --port 8880" -WindowStyle Minimized
...
Write-Host "
Text (Enter twice to submit):" -F Yellow;$tl=@();$f=$true
while($true){if($f){$l=Read-Host "  >";$f=$false}else{$l=Read-Host "  > (Enter to submit)"};if(!$l.Trim()){break};$tl+=$l}
$t=$tl -join "
";if(!$t.Trim()){Write-Host "No text" -F Red;Read-Host;exit 0}
Write-Host "
Generating..." -F Yellow
try{$b=@{model="kokoro";voice=$sv;input=$t;response_format="mp3"}|ConvertTo-Json;$r=Invoke-WebRequest "http://127.0.0.1:8880/v1/audio/speech" -Method Post -ContentType "application/json" -Body $b -TimeoutSec 120;$ts=Get-Date -Format "yyyyMMdd_HHmmss";$o=Join-Path $env:USERPROFILE\Desktop "tts_$sv_$ts.mp3";[IO.File]::WriteAllBytes($o,$r.Content);Write-Host "Saved: $o" -F Green}catch{Write-Host "ERROR: $_" -F Red}
Read-Host "
Press Enter"