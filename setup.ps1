...
$C = "Yellow"; $G = "Green"; $R = "Red"; $W = "White"; $D = "DarkGray"
...
function dl($url,$out,$label) {
    ...
    $wc.DownloadProgressChanged += {
        if ($_.TotalBytesToReceive -gt 0) {
            ...
            Write-Host ("`r  {0}% {1}  {2}/{3} MB  {4} MB/s  ETA {5}s" -f $p,$label,$mbD,$mbT,$sp,$eta) -NoNewline -F $W
        }
    }
    $wc.DownloadFile($url, $out); $sw.Stop()
    $ok = (Test-Path $out) -and (Get-Item $out).Length -gt 0
    if ($ok) { Write-Host "`r  done  $label" -F $G } else { Write-Host "`r  FAILED  $label" -F $R }
    return $ok
}
...
function spin {
    ...
    while ($job.State -eq "Running") {
        ...
        Write-Host ("`r  {0} {1}  {2}" -f $chars[$i%4], $label, $ts) -NoNewline -F $W
        Start-Sleep -Milliseconds 200; $i++
    }
    $sw.Stop(); $el = [math]::Floor($sw.Elapsed.TotalSeconds)
    $ts = if ($el -gt 60) { "$([math]::Floor($el/60))m$($el%60)s" } else { "${el}s" }
    $result = $job | Receive-Job; Remove-Job $job -Force
    if ($job.State -eq "Completed") { Write-Host "`r  done  $label  $ts" -F $G } else { Write-Host "`r  FAILED  $label" -F $R; throw $result }
}

function phase($t) { Clear-Host; Write-Host "`n  Verity JE Setup - $t`n" -F $C }

# ================================================================
# PHASE 1: DETECT
# ================================================================
while ($true) {
    Clear-Host; Write-Host "`n  Verity JE Setup - System Detection`n" -F $C
    ...
    Write-Host "  Git    " -NoNewline -F $D; Write-Host $(if($hasGit){"found"}else{"missing"}) -F $(if($hasGit){$G}else{$R})
    Write-Host "  Python " -NoNewline -F $D; Write-Host $(if($hasPy){$pyVer}else{"missing"}) -F $(if($hasPy){$G}else{$R})
    Write-Host "  uv     " -NoNewline -F $D; Write-Host $(if($hasUv){"found"}else{"missing"}) -F $(if($hasUv){$G}else{$R})
    Write-Host "  GPU    " -NoNewline -F $D; Write-Host $(if($hasGPU){"$gpuName ($vramGB GB)"}else{"CPU only"}) -F $(if($hasGPU){$G}else{$D})
    Write-Host "  RAM    " -NoNewline -F $D; Write-Host "$ramGB GB" -F $W
    Write-Host "`n  [Enter] continue  [Q] quit" -F $D; $k = K; if ($k.Key -eq "Q") { exit 0 }; if ($k.Key -eq "Enter") { break }
}
...
while ($true) {
    Clear-Host; Write-Host "`n  Verity JE Setup - Service Selection`n" -F $C
    ...
    for ($i = 0; $i -lt 3; $i++) {
        ...
        Write-Host "  $cur $chk " -NoNewline -F $W
        Write-Host $items[$i][0].PadRight(15) -NoNewline -F $W
        Write-Host $items[$i][1] -F $D
    }
    Write-Host "`n  [Up/Down] move  [Space] toggle  [Enter] confirm  [B] back  [Q] quit" -F $D
    $k = K
    ...
    if ($k.Key -eq "Enter") { $any = $svc.K -or $svc.L -or $svc.W; if (!$any) { Write-Host "`n  Select at least one service." -F $R; Start-Sleep 1 } else { break } }
}
...
# ================================================================
while ($true) {
    Clear-Host; Write-Host "`n  Verity JE Setup - Confirm`n" -F $C; Write-Host "  Will install:`n" -F $W
    if ($svc.K) { Write-Host "    [X] FastKoko        ~1.5 GB  TTS (Kokoro-82M)" -F $G }
    if ($svc.L) { Write-Host "    [X] LiteLLM         ~200 MB  AI Gateway" -F $G }
    if ($svc.W) { Write-Host "    [X] WhisperServer   ~3 GB    STT (Whisper)" -F $G }
    if (!$hasGit) { Write-Host "`n  Git will be installed." -F $R }
    if (!$hasUv) { Write-Host "  uv will be installed." -F $R }
    Write-Host "`n  Path: $Path" -F $D
    Write-Host "`n  [Y] proceed  [B] back  [Q] quit" -F $D
    $k = K
    if ($k.Key -eq "Q") { exit 0 }
    if ($k.Key -eq "B") { break }
...
if (!$hasGit -or !$hasUv) {
    ...
    if (!$hasGit) { Write-Host "  Installing Git..." -F $W; winget install --id Git.Git -e --silent --accept-package-agreements --accept-source-agreements 2>&1 | Out-Null; if (T git) { Write-Host "  Git installed" -F $G } else { Write-Host "  Git FAILED" -F $R; Read-Host; exit 1 } }
    if (!$hasUv) { Write-Host "  Installing uv..." -F $W; winget install --id AstralSoftware.uv -e --silent --accept-package-agreements --accept-source-agreements 2>&1 | Out-Null; if (T uv) { Write-Host "  uv installed" -F $G } else { Write-Host "  uv FAILED" -F $R; Read-Host; exit 1 } }
    Write-Host "`n  Press any key..." -F $D; K | Out-Null
}
...
if ($svc.K) {
    ...
    if (Test-Path (Join-Path $kDir "api\src\main.py")) { Write-Host "  Repository  already cloned" -F $G }
    ...
    if (Test-Path $kPy) { Write-Host "  Environment already exists" -F $G }
    ...
    if (Test-Path $kModel) { Write-Host "  Model  already downloaded" -F $G }
...
if ($svc.L) {
    ...
    if (T litellm) { $v = & litellm --version 2>&1; Write-Host "  LiteLLM  $v" -F $G }
    else { spin "Installing LiteLLM" { param($P,$S); uv tool install "litellm[proxy]" 2>&1 | Out-Null }; if (T litellm) { Write-Host "  LiteLLM  installed" -F $G } else { Write-Host "  LiteLLM  FAILED" -F $R } }
    $ub = "$env:USERPROFILE\.local\bin"; if ((Test-Path $ub) -and ($env:Path -notlike "*$ub*")) { $env:Path += ";$ub" }
    if (T ollama) { Write-Host "  Ollama  already installed" -F $G }
    else {
        Write-Host "`n  Ollama runs LLMs locally (private, offline)." -F $W
        Write-Host "  [Y] install  [N] skip" -F $D; $k = K
        if ($k.KeyChar -eq 'y') {
            ...
            if (T ollama) { Write-Host "  Ollama  installed" -F $G } else { Write-Host "  Ollama  restart terminal to use" -F $D }
        } else { Write-Host "  Ollama  skipped" -F $D }
    }
    if (T ollama) {
        Write-Host "`n  Pull a model? (e.g. llama3.2, gemma) Enter = skip" -F $W
        Write-Host "  > " -NoNewline -F $D; $m = Read-Host
        if ($m) { spin "Pulling $m" { param($P,$S); & ollama pull $args[0] 2>&1 | Out-Null } -ArgumentList $m | Out-Null; Write-Host "  Model $m pulled" -F $G }
    }
    Write-Host "`n  Press any key..." -F $D; K | Out-Null
}
...
if ($svc.W) {
    ...
    if (Test-Path $wPy) { Write-Host "  Environment already exists" -F $G }
    ...
    if (Test-Path $wFix) { $c = Get-Content $wFix -Raw; if ($c -notmatch "msvcrt") { $c = $c -replace "libc_name = ctypes.util.find_library\('c'\)", 'libc_name = "msvcrt.dll"'; $c | Set-Content $wFix -Enc UTF8 } }
    $dev = if ($hasGPU) { "cuda" } else { "cpu" }
    spin "Downloading Whisper model ($wModel, $dev)" { param($P,$S); Set-Location (Join-Path $P "WhisperServer"); $r = & ".venv\Scripts\python.exe" -c "import whisper; whisper.load_model('$($args[0])',device='$($args[1])');print('OK')" 2>&1; if ($r -notmatch "OK") { throw $r } } -ArgumentList $wModel, $dev
    Set-Location $startDir
    Write-Host "`n  Press any key..." -F $D; K | Out-Null
}
...
Write-Host "  Creating .bat and .ps1 files..." -F $W
. "$PSScriptRoot\_generate_scripts.ps1" -VerityTMPath $Path -WhisperModel $wModel
Write-Host "  Scripts generated" -F $G
Write-Host "`n  Press any key..." -F $D; K | Out-Null

Clear-Host; Write-Host "`n  Verity JE Setup - Complete!`n" -F $C
Write-Host "  Location: $Path`n" -F $D
if ($svc.K) { Write-Host "  FastKoko (TTS) -> http://127.0.0.1:8880/v1/   FastKoko.bat" -F $G }
if ($svc.L) { Write-Host "  LiteLLM (AI)  -> http://127.0.0.1:4000/v1/   LiteLLM.bat" -F $G }
if ($svc.W) { Write-Host "  Whisper (STT) -> http://127.0.0.1:9000/v1/   WhisperServer.bat ($wModel)" -F $G }
Write-Host "`n  [Y] Launch Manager  [N] Exit" -F $D; $k = K
if ($k.KeyChar -eq 'y' -or $k.KeyChar -eq 'Y') { Start-Process powershell -Arg "-NoExit","-EP","Bypass","-File",(Join-Path $Path "Manager.bat") }
else { Write-Host "`n  To start later: cd `"$Path`" ; .\Manager.bat" -F $D }
Write-Host "`n  Press any key." -F $D; K | Out-Null
