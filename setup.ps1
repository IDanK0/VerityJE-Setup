...
function spn($label, $step, $total, [scriptblock]$sb, $xa = @()) {
    ...
    if ($job.State -eq "Completed") {
        ...
        Write-Host "`r  FAIL  ${pf}${label}" -F $Rd; Write-Host "  ERROR: $result" -F $Rd; Read-Host; exit 1
    }
}
...
# Check if pip package is installed in a venv
function Pip-Has($venvDir, $pkg) {
    $py = Join-Path $venvDir ".venv\Scripts\python.exe"
    if (Test-Path $py) { try { $r = & $py -c "import $pkg" 2>&1; return $LASTEXITCODE -eq 0 } catch { return $false } }
    return $false
}
...
# === FASTKOKO ===
if ($svc.K) {
    $kDir = Join-Path $Path "Kokoro-FastAPI"; $kPy = Join-Path $kDir ".venv\Scripts\python.exe"
    $kModel = Join-Path $kDir "api\src\models\v1_0\kokoro-v1_0.pth"
    phase "FastKoko - Kokoro TTS"

    if (Test-Path (Join-Path $kDir "api\src\main.py")) { Write-Host "  skip  [1/5] Repository (already cloned)" -F $Dg }
    else { spn "Clone repository" 1 5 { param($P, $S) Set-Location $S; & git clone https://github.com/remsky/Kokoro-FastAPI.git (Join-Path $P "Kokoro-FastAPI") 2>&1 | Out-Null; if (-not (Test-Path (Join-Path $P "Kokoro-FastAPI\api\src\main.py"))) { throw "Clone failed" } } }

    if (Test-Path $kPy) { Write-Host "  skip  [2/5] Environment (already exists)" -F $Dg }
    else { spn "Create Python environment ($bestPy)" 2 5 { param($P, $S, $py) $d = Join-Path $P "Kokoro-FastAPI"; Set-Location $d; $e = & uv venv .venv --python $py --seed 2>&1; if (-not (Test-Path (Join-Path $d ".venv\Scripts\python.exe"))) { throw "venv failed: $e" } } -xa $bestPy }

    if (Pip-Has $kDir "kokoro" ) { Write-Host "  skip  [3/5] Dependencies (already installed)" -F $Dg }
    else {
        $env:CUDA = $cudaIdx
    ...
    if (Test-Path $kModel) { Write-Host "  skip  [4/5] Model (already downloaded)" -F $Dg }
    else { spn "Download Kokoro model (350 MB)" 4 5 { param($P, $S) $wc = New-Object Net.WebClient; $wc.DownloadFile("https://huggingface.co/hexgrad/Kokoro-82M/resolve/main/kokoro-v1_0.pth", (Join-Path $P "Kokoro-FastAPI\api\src\models\v1_0\kokoro-v1_0.pth")) } }
...
if ($svc.L) {
    ...
    if (T litellm) { $v = & litellm --version 2>&1; Write-Host "  skip  [1/2] LiteLLM ($v)" -F $Dg }
    else { spn "Install LiteLLM" 1 2 { param($P, $S) uv tool install "litellm[proxy]" 2>&1 | Out-Null }; Write-Host "  done  [1/2] LiteLLM installed" -F $Gn }
    if ($uvBin -and ($env:Path -notlike "*$uvBin*")) { $env:Path += ";$uvBin" }
    if (T ollama) { Write-Host "  skip  [2/2] Ollama (already installed)" -F $Dg }
    else {
        ...
        if ($k.KeyChar -eq 'y') {
            ...
            spn "Download Ollama" 2 2 { param($P, $S) $wc = New-Object Net.WebClient; $wc.DownloadFile("https://ollama.com/download/ollama-windows-amd64.exe", (Join-Path $env:TEMP "OllamaSetup.exe")) }
...
# === WHISPER ===
if ($svc.W) {
    $wDir = Join-Path $Path "WhisperServer"; $wPy = Join-Path $wDir ".venv\Scripts\python.exe"
    ...
    if (Test-Path $wPy) { Write-Host "  skip  [1/3] Environment (already exists)" -F $Dg }
    else { spn "Create Python environment ($bestPy)" 1 3 { param($P, $S, $py) $d = Join-Path $P "WhisperServer"; Set-Location $d; $e = & uv venv .venv --python $py --seed 2>&1; if (-not (Test-Path (Join-Path $d ".venv\Scripts\python.exe"))) { throw "venv failed: $e" } } -xa $bestPy }

    if (Pip-Has $wDir "whisper") { Write-Host "  skip  [2/3] Dependencies (already installed)" -F $Dg }
    else {
        $env:CUDA = $cudaIdx
