...
if ($svc.K) { $kDir = Join-Path $Path "Kokoro-FastAPI"; $kPy = Join-Path $kDir ".venv\Scripts\python.exe"; $kModel = Join-Path $kDir "api\src\models\v1_0\kokoro-v1_0.pth"
    ...
    if (Test-Path $kPy) { Write-Host "  skip  [2/5] Environment" -F $Dg } else { Write-Host "  [2/5] Creating Python environment ($bestPy)..." -F $Wh; Set-Location $kDir; $e = & uv venv .venv --python $bestPy --seed 2>&1; if (-not (Test-Path ".venv\Scripts\python.exe")) { Write-Host "  FAIL: $e" -F $Rd; Read-Host; exit 1 }; Set-Location $startDir; Write-Host "  done  [2/5] Environment" -F $Gn }
    if (Pip-Has $kDir "kokoro") { Write-Host "  skip  [3/5] Dependencies" -F $Dg } else { Write-Host "  [3/5] Installing dependencies (torch + kokoro)..." -F $Wh; Set-Location $kDir; $pip = ".venv\Scripts\pip.exe"; & $pip install --upgrade pip -q 2>&1 | Out-Null; & $pip install "cython<3.0" -q 2>&1 | Out-Null; & $pip install -e ".[cpu]" 2>&1 | Out-Null; & $pip uninstall torch -y -q 2>&1 | Out-Null; & $pip install torch --index-url "https://download.pytorch.org/whl/$cudaIdx" --timeout 600 -q 2>&1 | Out-Null; Set-Location $startDir; Write-Host "  done  [3/5] Dependencies" -F $Gn }
    Set-Location $startDir
    if (Test-Path $kModel) { Write-Host "  skip  [4/5] Model" -F $Dg } else { Write-Host "  [4/5] Downloading Kokoro model (350 MB)..." -F $Wh; $wc = New-Object Net.WebClient; $wc.DownloadFile("https://huggingface.co/hexgrad/Kokoro-82M/resolve/main/kokoro-v1_0.pth", $kModel); Write-Host "  done  [4/5] Model" -F $Gn }
...
if ($svc.W) { $wDir = Join-Path $Path "WhisperServer"; $wPy = Join-Path $wDir ".venv\Scripts\python.exe"; New-Item -ItemType Dir -Path $wDir -Force | Out-Null
    ...
    if (Test-Path $wPy) { Write-Host "  skip  [1/3] Environment" -F $Dg } else { Write-Host "  [1/3] Creating Python environment ($bestPy)..." -F $Wh; Set-Location $wDir; $e = & uv venv .venv --python $bestPy --seed 2>&1; if (-not (Test-Path ".venv\Scripts\python.exe")) { Write-Host "  FAIL: $e" -F $Rd; Read-Host; exit 1 }; Set-Location $startDir; Write-Host "  done  [1/3] Environment" -F $Gn }
    if (Pip-Has $wDir "whisper") { Write-Host "  skip  [2/3] Dependencies" -F $Dg } else { Write-Host "  [2/3] Installing dependencies (whisper + torch)..." -F $Wh; Set-Location $wDir; $pip = ".venv\Scripts\pip.exe"; & $pip install --upgrade pip -q 2>&1 | Out-Null; & $pip install "openai-whisper>=1.1.10" "uvicorn[standard]" "fastapi" "pydantic" "python-multipart" "mutagen" -q 2>&1 | Out-Null; if ($hasGPU) { & $pip uninstall torch -y -q 2>&1 | Out-Null; & $pip install torch --index-url "https://download.pytorch.org/whl/$cudaIdx" --timeout 600 -q 2>&1 | Out-Null }; Set-Location $startDir; Write-Host "  done  [2/3] Dependencies" -F $Gn }
    ...
    Write-Host "  [3/3] Downloading Whisper model ($wModel, $dev)..." -F $Wh
    Set-Location $wDir
    $r = & $wPy -c "import whisper; whisper.load_model('$wModel',device='$dev');print('OK')" 2>&1
    Set-Location $startDir
    if ($r -match "OK") { Write-Host "  done  [3/3] Model loaded" -F $Gn } else { Write-Host "  FAIL  [3/3] Model: $r" -F $Rd; Read-Host; exit 1 }
    wait
}
