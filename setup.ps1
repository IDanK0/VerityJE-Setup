...
# ================================================================
# SPINNER - runs a scriptblock while showing animated spinner
# ================================================================
function spn($label,$step,$total,[scriptblock]$sb){
    $job=Start-Job -ScriptBlock $sb -ArgumentList $Path,$startDir
    $c=@("\","|","/","-");$i=0;$sw=[Diagnostics.Stopwatch]::StartNew()
    $prefix = if($total -gt 0){"[$step/$total] "}else{""}
    while($job.State -eq "Running"){
        $el=[math]::Floor($sw.Elapsed.TotalSeconds)
        $ts=if($el -gt 60){"$([math]::Floor($el/60))m$($el%60)s"}else{"${el}s"}
        Write-Host ("`r  [{0}] ${prefix}${label}...  ({1})" -f $c[$i%4],$ts) -NoNewline -F $Wh
        Start-Sleep -Milliseconds 200;$i++
    }
    $el=[math]::Floor($sw.Elapsed.TotalSeconds)
    $ts=if($el -gt 60){"$([math]::Floor($el/60))m$($el%60)s"}else{"${el}s"}
    $result=$job|Receive-Job;Remove-Job $job -Force
    if($job.State -eq "Completed"){Write-Host "`r  done  ${prefix}${label}  (${ts})" -F $Gn}else{Write-Host "`r  FAIL  ${prefix}${label}" -F $Rd;throw$result}
}

function dwn($url,$out,$label,$step,$total){
    $prefix = if($total -gt 0){"[$step/$total] "}else{""}
    spn "Downloading $label" $step $total {param($P,$S);$wc=New-Object Net.WebClient;$wc.DownloadFile($url,$out);if(!(Test-Path $out) -or (Get-Item $out).Length -lt 1){throw"download failed"}}
}
...
while($true){
    ...
    if($k.Key -eq "B"){break}
    if($k.KeyChar -eq 'y' -or $k.KeyChar -eq 'Y'){break}
}
...
if($svc.K){
    ...
    else{spn "Cloning Kokoro-FastAPI" 1 5 {param($P,$S);Set-Location$S;& git clone https://github.com/remsky/Kokoro-FastAPI.git(Join-Path$P"Kokoro-FastAPI")2>&1|Out-Null;if(!(Test-Path(Join-Path$P"Kokoro-FastAPI\api\src\main.py"))){throw"Clone failed"}}}
    if(Test-Path$kPy){Write-Host"  done  [2/5] Environment  (already exists)" -F $Gn}
    else{spn "Creating Python 3.10 environment" 2 5 {param($P,$S);Set-Location(Join-Path$P"Kokoro-FastAPI");& uv venv .venv --python 3.10 --seed 2>&1|Out-Null;if(!(Test-Path".venv\Scripts\python.exe")){throw"venv failed"}}}
    spn "Installing dependencies (torch + kokoro)" 3 5 {param($P,$S);Set-Location(Join-Path$P"Kokoro-FastAPI");$pip=".venv\Scripts\pip.exe";& $pip install --upgrade pip -q 2>&1|Out-Null;& $pip install "cython<3.0" -q 2>&1|Out-Null;& $pip install -e ".[cpu]" 2>&1|Out-Null}
    Set-Location$startDir
    if(Test-Path$kModel){Write-Host"  done  [4/5] Model  (already downloaded)" -F $Gn}
    else{dwn "https://huggingface.co/hexgrad/Kokoro-82M/resolve/main/kokoro-v1_0.pth" $kModel "Kokoro model (350 MB)" 4 5}
    Write-Host"  [5/5] Ready" -F $Gn
    Write-Host"`n  Press any key..." -F $Dg;K|Out-Null
}

# ================================================================
# PHASE 6: LITELLM (3 steps)
# ================================================================
if($svc.L){
    phase "LiteLLM - AI Gateway"
    if(T litellm){$v=& litellm --version 2>&1;Write-Host"  done  [1/3] LiteLLM  $v" -F $Gn}
    else{spn "Installing LiteLLM" 1 3 {param($P,$S);uv tool install "litellm[proxy]" 2>&1|Out-Null};Write-Host"  done  [1/3] LiteLLM  installed" -F $Gn}
    $ub="$env:USERPROFILE\.local\bin";if((Test-Path$ub)-and($env:Path -notlike "*$ub*")){$env:Path+=";$ub"}
    if(T ollama){Write-Host"  done  [2/3] Ollama  already installed" -F $Gn}
    else{
        Write-Host"`n  Ollama runs LLMs locally (private, offline)." -F $Wh
        Write-Host"  [Y] install  [N] skip" -F $Dg;$k=K
        if($k.KeyChar -eq 'y'){$tmp=Join-Path$env:TEMP"OllamaSetup.exe";dwn"https://ollama.com/download/ollama-windows-amd64.exe"$tmp"Ollama"0 0;if(Test-Path$tmp){Start-Process -FilePath$tmp -Arg"/S" -Wait -EA SilentlyContinue;Remove-Item$tmp -EA SilentlyContinue;$env:Path=[Environment]::GetEnvironmentVariable("Path","Machine")+";"+[Environment]::GetEnvironmentVariable("Path","User")};if(T ollama){Write-Host"  done  [2/3] Ollama  installed" -F $Gn}else{Write-Host"  done  [2/3] Ollama  restart terminal to use" -F $Dg}}else{Write-Host"  skip  [2/3] Ollama  skipped" -F $Dg}
    }
    if(T ollama){Write-Host"`n  Pull a model? (e.g. llama3.2) Enter = skip" -F $Wh;Write-Host"  > " -NoNewline -F $Dg;$m=Read-Host;if($m){spn"Pulling $m"3 3{param($P,$S);& ollama pull$args[0]2>&1|Out-Null}-ArgumentList$m|Out-Null;Write-Host"  done  [3/3] Model pulled" -F $Gn}else{Write-Host"  skip  [3/3] Model  skipped" -F $Dg}}
    Write-Host"`n  Press any key..." -F $Dg;K|Out-Null
}

# ================================================================
# PHASE 7: WHISPER (4 steps)
# ================================================================
if($svc.W){
    ...
    spn "Downloading Whisper model ($wModel, $dev)" 3 4 {param($P,$S);Set-Location(Join-Path$P"WhisperServer");$r=& ".venv\Scripts\python.exe" -c "import whisper; whisper.load_model('$($args[0])',device='$($args[1])');print('OK')" 2>&1;if($r -notmatch"OK"){throw$r}} -ArgumentList$wModel,$dev
    Set-Location$startDir
    Write-Host"  [4/4] Ready" -F $Gn
    Write-Host"`n  Press any key..." -F $Dg;K|Out-Null
}
...
if($k.KeyChar -eq 'y' -or$k.KeyChar -eq 'Y'){Start-Process powershell -Arg"-NoExit","-EP","Bypass","-File",(Join-Path$Path"Manager.bat")}else{Write-Host"`n  To start later: cd `"$Path`" ; .\Manager.bat" -F $Dg}
Write-Host"`n  Press any key." -F $Dg;K|Out-Null
